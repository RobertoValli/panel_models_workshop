...
\usepackage{csquotes}
\usepackage[style=apa, maxcitenames=3]{biblatex}
...
That said, scholars proposed ways to estimate consistent ATTs when parallel trends are valid conditional on a set of covariates \parencite{Sant'Anna2020}. The authors proposed a family of estimators that have desirable properties when either the outcome model or the covariate set are correctly specified.
...
\hlcom{# Create first-difference of treatement and outcome}
...
For settings that go beyond the typical policy implementation where a set of treated units receives an absorbing state treatment, methodologists have developed
